    ...
    methods
        function result=run(~,problem,pair,options,context)
            ...
            sourcePair=struct('First',pair.First.toStruct(), ...
                'Second',pair.Second.toStruct(), ...
                'RequestedRadius',pair.RequestedRadius, ...
                'AchievedRadius',pair.AchievedRadius, ...
            ...
            result=lmz.data.ContinuationResult(branch,snapshots,reason, ...
                options.toStruct(),diagnostics,sourcePair,context.RandomSeed, ...
                struct('algorithm','pseudo-arclength', ...
            ...
            if ~isempty(options.CheckpointPath)
                saveCheckpoint(options.CheckpointPath,branch,options, ...
                    stats.FinalStep,reason,snapshots,sourcePair, ...
                    context.RandomSeed,toc(started));
            end
                ...
                while numel(values)<count
                    ...
                    if accepted
                        ...
                        if ~isempty(options.CheckpointPath),saveCheckpoint( ...
                            ...
                            options,step,'running',localSnapshots,sourcePair, ...
                            context.RandomSeed,toc(started));end
...
function saveCheckpoint(path,branch,options,step,reason,snapshots,sourcePair,randomSeed,elapsedTime)
...
artifact=lmz.io.ArtifactStore.withRunMetadata(artifact,struct( ...
    'Options',options.toStruct(),'SourcePair',sourcePair, ...
    'RandomSeed',randomSeed,'Provenance',struct( ...
    'algorithm','pseudo-arclength','matlabVersion',version), ...
    'ElapsedTime',elapsedTime,'FunctionEvaluations',NaN, ...
