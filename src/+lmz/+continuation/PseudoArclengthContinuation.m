        ...
        function result=run(~,problem,pair,options,context)
            ...
            function [values,localSnapshots,termination,traceStats]=trace(first,second,count,directionSign)
                ...
                while numel(values)<count
                    ...
                    active=values(end);
                    coordinates=coordinateDiagnostics(problem, ...
                        active.DecisionValues,prediction,[]);
                    preview=struct('Kind','prediction','PointIndex',numel(values)+1, ...
                        ...
                        'ActiveDecision',coordinates.Active, ...
                        'Prediction',coordinates.Predicted, ...
                        'CorrectedDecision',coordinates.Corrected, ...
                        'CoordinateDiagnostics',coordinates, ...
                        'Gait',active.Classification, ...
                    ...
                    if accepted
                        ...
                        if any(distances<options.DuplicateTolerance)
                            ...
                            diagnostics=attemptDiagnostics(prediction,corrected, ...
                                residualNorm,NaN,output,backtracks,feasibility,gait, ...
                                termination,'history-duplicate',directionSign,NaN, ...
                                exitFlag,active.DecisionValues);
                            localSnapshots(end+1,1)=lmz.data.ContinuationSnapshot( ...
                        ...
                        diagnostics=attemptDiagnostics(prediction,corrected, ...
                            ...
                            directionSign,achieved,exitFlag,active.DecisionValues);
            ...
            function value=attemptDiagnostics(prediction,corrected,residualNorm, ...
                    ...
                    activeDecision)
                coordinates=coordinateDiagnostics(problem,activeDecision, ...
...
function value=coordinateDiagnostics(problem,active,predicted,corrected)
%COORDINATEDIAGNOSTICS Named decision coordinates for generic consumers.
value=struct('Names',{problem.getDecisionSchema().names()}, ...
    'Active',active(:),'Predicted',predicted(:),'Corrected',corrected(:));
end
