classdef Solution
    ...
    methods
        ...
        function artifact=toArtifact(obj)
            problemMetadata=lmz.data.Solution.resolveProblemMetadata(obj);
            ...
            artifact=struct('schemaVersion',artifactSchemaVersion, ...
                ...
                'randomSeed',0,'sourceCommitSHAs', ...
                lmz.data.Solution.provenanceFields(obj.Provenance,'commit'), ...
                'sourceDataHashes',lmz.data.Solution.provenanceFields( ...
                struct('Provenance',obj.Provenance,'Lineage',obj.Lineage, ...
                'Diagnostics',obj.Diagnostics),'hash'), ...
                'createdAt',obj.CreatedAt, ...
                'matlabVersion',version,'codeVersion',frameworkVersion, ...
                'problemMaturity',problemMetadata.maturity, ...
                'validationStatus',problemMetadata.validationStatus, ...
                'problemMetadata',problemMetadata,'solution',obj.toStruct());
        end
    end
    methods (Static)
    ...
    methods (Static, Access=private)
        function metadata=resolveProblemMetadata(solution)
            try
                registry=lmz.registry.ModelRegistry.discover();
                metadata=registry.getProblemDescriptor( ...
                    solution.ModelId,solution.ProblemId);
            catch
                metadata=struct('id',solution.ProblemId, ...
                    'maturity','experimental', ...
                    'validationStatus','untested', ...
                    'provenance',struct('source','unregistered-problem'), ...
                    'capabilities',struct());
            end
        end

        function values=provenanceFields(source,kind)
            values=struct();values=collect(source,'',values,kind);
            function output=collect(value,prefix,output,filter)
                if ~isstruct(value)||~isscalar(value),return,end
                names=fieldnames(value);
                for fieldIndex=1:numel(names)
                    name=names{fieldIndex};item=value.(name);
                    key=name;if ~isempty(prefix),key=[prefix '_' name];end
                    lowerName=lower(name);
                    if isstruct(item)&&isscalar(item)
                        output=collect(item,key,output,filter);
                    elseif (ischar(item)||(isstring(item)&&isscalar(item)))&& ...
                            ((strcmp(filter,'commit')&&contains(lowerName,'commit'))|| ...
                            (strcmp(filter,'hash')&&(contains(lowerName,'hash')||contains(lowerName,'sha'))&&~contains(lowerName,'commit')))
                        output.(matlab.lang.makeValidName(key))=char(item);
                    end
                end
            end
        end
