...
violations={};counts=struct('Files',numel(files),'UIComponents',0, ...
    ...
    'VideoWriterCalls',0,'GetframeCalls',0,'RecursiveDirCalls',0, ...
...
for fileIndex=1:numel(files)
    ...
    getframeCalls=countMatches(code,'\<getframe\s*\(');
    counts.GetframeCalls=counts.GetframeCalls+getframeCalls;
    if getframeCalls>0&&runtimeFile&&~compatibilityFile
        violations{end+1}=sprintf( ... %#ok<AGROW>
            '%s: getframe must route through lmz.compat.Graphics.',relative);
    end
