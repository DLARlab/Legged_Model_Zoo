...
addParameter(parser, 'AuthoringRoute', 'minimal_simulation', ...
...
route = canonicalAuthoringRoute(parser.Results.AuthoringRoute);
...
mapping = templateMapping(modelId, route);
...
targets = { ...
    ...
    'plugin.json'};
if strcmp(route, 'scientific_periodic_branch')
    targets{end + 1, 1} = fullfile('examples', ...
        ['demo_' modelId '_registered_workflow.m']);
end
...
function route = canonicalAuthoringRoute(value)
value = lmz.compat.Text.character(value, 'authoring route');
switch value
    case {'minimal','minimal_simulation'}
        route = 'minimal_simulation';
    case {'scientific','scientific_periodic_branch'}
        route = 'scientific_periodic_branch';
    otherwise
        error('lmz:Template:AuthoringRoute', ...
            ['AuthoringRoute must be minimal_simulation or ' ...
...
function mapping = templateMapping(modelId, route)
package = fullfile('models', '+lmzmodels', ['+' modelId]);
catalog = fullfile('catalog', modelId);
manifestTemplate = fullfile('catalog', 'manifest.json.in');
if strcmp(route, 'scientific_periodic_branch')
    manifestTemplate = fullfile('catalog', 'manifest_scientific.json.in');
end
mapping = { ...
...
if strcmp(route, 'scientific_periodic_branch')
    mapping = [mapping; { ...
        fullfile('model', 'BranchCatalogProvider.m.in'), ...
            fullfile(package, 'BranchCatalogProvider.m'); ...
        fullfile('model', 'LegacyAdapterProvider.m.in'), ...
            fullfile(package, 'LegacyAdapterProvider.m'); ...
        fullfile('catalog', 'data_sources.lmz.json.in'), ...
            fullfile(catalog, 'data_sources.lmz.json'); ...
        fullfile('catalog', 'workbench.lmz.json.in'), ...
            fullfile(catalog, 'workbench.lmz.json'); ...
        fullfile('catalog', 'workflow.lmz.json.in'), ...
            fullfile(catalog, 'workflows', ...
            'periodic_branch_workflow.json'); ...
        fullfile('tests', 'TestGeneratedScientificWorkflow.m.in'), ...
            fullfile('tests', 'generated', modelId, ...
            'TestGeneratedScientificWorkflow.m'); ...
        fullfile('examples', 'demo_registered_workflow.m.in'), ...
            fullfile('examples', ...
            ['demo_' modelId '_registered_workflow.m'])}];
end
end
