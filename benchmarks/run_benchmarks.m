...
tutorialModel = registry.createModel('tutorial_hopper');
tutorialProblem = tutorialModel.createProblem('periodic_hop', struct());
tutorialDecision = tutorialProblem.getDecisionSchema().defaults();
tutorialParameters = tutorialProblem.getParameterSchema().defaults();
tutorialEvaluation = tutorialProblem.evaluate(tutorialDecision, ...
    tutorialParameters, context, true);
tutorialCatalog = registry.getPoincareSectionRegistry('tutorial_hopper');
descendingSection = tutorialCatalog.section('height_descending');
heightIndex = tutorialEvaluation.Simulation.StateSchema.indexOf('y');
crossingIndex = find( ...
    tutorialEvaluation.Simulation.States(1:end-1, heightIndex) > 0.1 & ...
    tutorialEvaluation.Simulation.States(2:end, heightIndex) <= 0.1, 1);
crossingTimes = tutorialEvaluation.Simulation.Time( ...
    crossingIndex:crossingIndex + 1);
crossingStates = tutorialEvaluation.Simulation.States( ...
    crossingIndex:crossingIndex + 1, :).';
tutorialTimingProblem = tutorialModel.createProblem( ...
    'section_return_timing', struct());

loadCatalog = lmzmodels.slip_quad_load.ScientificDatasetCatalog.default();
loadTransition = loadCatalog.load(loadCatalog.Manifest.defaultMultiStride);
completionRequest = lmz.multistride.MultiStrideRequest( ...
    'NumberOfStrides', 5, 'InitialDecision', loadTransition.XAccum, ...
    'CompletionPolicy', 'carry_forward');
nStrideFit = loadModel.createProblem('n_stride_fit', struct());
nStrideFitDecision = nStrideFit.getDecisionSchema().defaults();
nStrideFitParameters = nStrideFit.getParameterSchema().defaults();
...
definitions = { ...
    ...
    definition('poincare_crossing_detection_1000', ...
        @() detectSectionCrossings(), ...
        'tutorial descending-height section, 1000 detections', 5); ...
    definition('tutorial_contact_timing_solve', @() solveTutorialTiming(), ...
        'fixed-state/fixed-physics tutorial apex return', 15); ...
    definition('stride_plan_completion_three_added', ...
        @() completeThreeStrideSpecs(), ...
        'quad-load structural carry-forward from two to five strides', 10); ...
    definition('tutorial_three_stride_simulation', ...
        @() simulateTutorialStrides(3), ...
        'tutorial requested three-stride simulation', 15); ...
    definition('tutorial_five_stride_simulation', ...
        @() simulateTutorialStrides(5), ...
        'tutorial requested five-stride simulation', 20); ...
    definition('n_stride_objective_evaluation', ...
        @() evaluateNStrideObjective(), ...
        'quad-load two-stride fixed-timing objective', 20); ...
    definition('gui_stride_plan_refresh_20', @() refreshStridePlan(), ...
        'shared GUI stride-plan controls, 20 refreshes', 30)};
...
fprintf('LMZ_BENCHMARKS_OK records=%d release=%s\n', ...
    ...
    function value = detectSectionCrossings()
        accepted = false(1000, 1);
        for crossingRepeat = 1:numel(accepted)
            [detected, crossing] = descendingSection.detectCrossing( ...
                crossingTimes(1), crossingStates(:, 1), ...
                crossingTimes(2), crossingStates(:, 2));
            accepted(crossingRepeat) = detected && crossing.Accepted;
        end
        value = struct('Count', numel(accepted), ...
            'AcceptedCount', sum(accepted));
    end
    function value = solveTutorialTiming()
        value = lmz.services.ContactTimingService().solve( ...
            tutorialTimingProblem, ...
            tutorialTimingProblem.getDecisionSchema().defaults(), ...
            struct(), context);
    end
    function value = completeThreeStrideSpecs()
        value = lmzmodels.slip_quad_load.QuadLoadStridePlanBuilder().build( ...
            completionRequest, context);
    end
    function value = simulateTutorialStrides(count)
        request = lmz.multistride.MultiStrideRequest( ...
            'NumberOfStrides', count, 'CompletionPolicy', 'carry_forward');
        value = lmz.services.MultiStrideSimulationService().simulate( ...
            tutorialModel, request, context);
    end
    function value = evaluateNStrideObjective()
        [objective, terms, diagnostics] = nStrideFit.evaluateObjective( ...
            nStrideFitDecision, nStrideFitParameters, context);
        value = struct('Objective', objective, 'Terms', terms, ...
            'Diagnostics', diagnostics);
    end
    function value = refreshStridePlan()
        application = lmz.gui.LeggedModelZooApp('Visible', 'off');
        cleanup = onCleanup(@() delete(application));
        application.Controller.selectModel('slip_quad_load');
        drawnow;
        tab = application.tab('simulation');
        for refreshIndex = 1:20
            tab.refresh();
        end
        value = tab.testHooks().RefreshCount;
        clear cleanup
    end
end
