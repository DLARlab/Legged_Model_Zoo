...
rectangularTimingProblem = tutorialModel.createProblem( ...
    'section_return_timing', struct('FixedEventMask', true, ...
    'FreeReturnTime', true, ...
    'FixedRowPolicy', 'include_fixed_rows_in_least_squares'));
shootingProblem2 = tutorialModel.createProblem( ...
    'multiple_shooting', struct('HorizonLength', 2));
shootingProblem3 = tutorialModel.createProblem( ...
    'multiple_shooting', struct('HorizonLength', 3));
shootingProblem5 = tutorialModel.createProblem( ...
    'multiple_shooting', struct('HorizonLength', 5));
shootingParameters = shootingProblem2.getParameterSchema().defaults();
shootingSeed2 = shootingProblem2.getDecisionSchema().defaults();
shootingSeed3 = shootingProblem3.getDecisionSchema().defaults();
shootingSeed5 = shootingProblem5.getDecisionSchema().defaults();
...
definitions = { ...
    ...
    definition('build_gui', @() buildGui(), ...
        'real uifigure and all GUI components', 30); ...
    ...
    definition('gui_stride_plan_refresh_20', @() refreshStridePlan(), ...
        'shared GUI stride-plan controls, 20 refreshes', 60); ...
    definition('rectangular_timing_solve', @() solveRectangularTiming(), ...
        'tutorial fixed-row overdetermined timing, m=2 and n=1', 20); ...
    definition('multiple_shooting_evaluate_n2', ...
        @() evaluateShooting(shootingProblem2, shootingSeed2), ...
        'analytic tutorial two-segment residual', 10); ...
    definition('multiple_shooting_evaluate_n3', ...
        @() evaluateShooting(shootingProblem3, shootingSeed3), ...
        'analytic tutorial three-segment residual', 10); ...
    definition('multiple_shooting_evaluate_n5', ...
        @() evaluateShooting(shootingProblem5, shootingSeed5), ...
        'analytic tutorial five-segment residual', 10); ...
    definition('multiple_shooting_finite_difference_jacobian_n2', ...
        @() shootingJacobian(), ...
        'scaled finite-difference Jacobian for two segments', 20); ...
    definition('multiple_shooting_periodic_correction_n2', ...
        @() correctShooting(), ...
        'rank-aware two-segment periodic correction', 30); ...
    definition('horizon_continuation_n2_to_n3', ...
        @() continueShootingHorizon(), ...
        'named dimension embedding and one horizon extension', 45); ...
    definition('gui_horizon_diagnostics_refresh_20', ...
        @() refreshHorizonDiagnostics(), ...
        'multiple-shooting GUI diagnostics, 20 refreshes', 60)};
...
if gateOnly
    keep = {'startup_registry', 'roadmap_one_branch', ...
        ...
        'rectangular_timing_solve', ...
        'multiple_shooting_evaluate_n5', ...
        'gui_horizon_diagnostics_refresh_20'};
...
fprintf('LMZ_BENCHMARKS_OK records=%d release=%s\n', ...
    ...
    function value = solveRectangularTiming()
        value = lmz.services.ContactTimingService().solve( ...
            rectangularTimingProblem, ...
            rectangularTimingProblem.getDecisionSchema().defaults(), ...
            struct('Solver', 'lsqnonlin', ...
            'ResidualTolerance', 1e-9), context);
    end
    function value = evaluateShooting(problem, seed)
        value = problem.evaluate(seed, shootingParameters, context, false);
    end
    function value = shootingJacobian()
        value = lmz.solvers.RankAwareNonlinearSolver().analyze( ...
            shootingProblem2, shootingSeed2, shootingParameters, ...
            struct(), context);
    end
    function value = correctShooting()
        value = lmz.services.MultipleShootingService().solve( ...
            shootingProblem2, shootingSeed2, ...
            struct('ResidualTolerance', 1e-9), context);
    end
    function value = continueShootingHorizon()
        configurations = {struct('HorizonLength', 2), ...
            struct('HorizonLength', 3)};
        value = lmz.services.HorizonContinuationService().run( ...
            tutorialModel, 'multiple_shooting', configurations, ...
            shootingSeed2, struct('SolverOptions', struct( ...
            'ResidualTolerance', 1e-9)), context);
    end
    function value = refreshHorizonDiagnostics()
        application = lmz.gui.LeggedModelZooApp('Visible', 'off');
        cleanup = onCleanup(@() delete(application));
        application.Controller.selectModel('tutorial_hopper');
        application.Controller.setSolveMode('Multiple shooting');
        application.Controller.setShootingSettings( ...
            struct('HorizonLength', 3));
        tab = application.tab('solve');
        for refreshIndex = 1:20
            tab.refresh();
        end
        hooks = tab.testHooks();
        value = struct('RefreshCount', hooks.RefreshCount, ...
            'DiagnosticsRows', size( ...
            hooks.Controls.ShootingDiagnosticsTable.Data, 1));
        clear cleanup
    end
end
