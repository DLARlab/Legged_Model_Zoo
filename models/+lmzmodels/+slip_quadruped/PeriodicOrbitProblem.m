classdef PeriodicOrbitProblem < lmz.api.NonlinearEquationProblem
    %PERIODICORBITPROBLEM Section-local configurable RoadMap orbit problem.
    %   The exact apex preset delegates unchanged to PeriodicApexProblem.
    %   Every other supported periodic section owns its initial section
    %   coordinates and cyclic contact schedule and is integrated directly.
    ...
    methods
        ...
        function evaluation=evaluate(obj,u,p,context,includeSimulation)
            ...
            % Keep a stable block position across named-event and state-plane
            % charts; a named-event endpoint has no independent scalar row.
            blocks(end+1,1)=lmz.data.ResidualBlock( ...
        ...
        function solution=makeSolution(obj,u,p,evaluation)
            ...
            if obj.ApexEquivalent
                ...
                classification=lmzmodels.slip_quadruped. ...
                    GaitClassifier.classify(localLegacyDecision( ...
                    obj.SectionCodec,u));
            end
...
catalog=registry.getPoincareSectionRegistry('slip_quadruped');
...
codec=lmzmodels.slip_quadruped.QuadrupedSectionDecisionCodec( ...
...
configuration.SectionDecisionCodec='quadruped-section-local-v1';
end
