classdef SectionContactConstraintProvider < ...
    ...
    %SECTIONCONTACTCONSTRAINTPROVIDER Direct non-apex jerboa timing rows.
    ...
    methods
        function obj=SectionContactConstraintProvider(codec,adapter)
            if ~isa(codec, ...
                    ...
                    ~isa(adapter,['lmzmodels.slip_biped.' ...
                    'BipedSectionSimulationAdapter'])
                error('lmz:Timing:BipedSectionProvider', ...
                    'Biped section timing requires its codec and adapter.');
            end
            obj.Codec=codec;
            obj.Adapter=adapter;
        end
        ...
        function value=evaluate(obj,initialState,physicalParameters, ...
            ...
            for index=1:numel(names)
                ...
                if isempty(target)
                    error('lmz:Timing:BipedSectionSchedule', ...
