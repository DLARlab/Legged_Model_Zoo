classdef BuiltInMultiStrideSimulationProvider < ...
        lmz.multistride.MultiStrideProvider
    %BUILTINMULTISTRIDESIMULATIONPROVIDER Model-owned built-in plan runtime.
    methods
        ...
        function report=previewEnergy(~,plan,count,energyNeutral, ...
                overrides,declaredWork,context)
            if isempty(plan)||~isa(plan,'lmz.multistride.StridePlan')|| ...
                    ~strcmp(plan.ModelId,'slip_quad_load')
                error('lmz:MultiStride:EnergyPreviewUnavailable', ...
                    ['Energy preview requires a registered load-pulling ' ...
                    'stride plan.']);
            end
            plan=plan.clone();
        ...
        function schedule=scheduleOverride(~,plan,index,timing)
            if ~isempty(plan)&&plan.CompletedStrideCount>0
                schedule=plan.StrideSpecs( ...
                    min(index,plan.CompletedStrideCount)).EventSchedule;
            else
                names=lmzmodels.slip_quad_load.LaterStrideLayout.baseNames();
                schedule=struct('Names',{names(:)},'Times',timing(:), ...
                    'ReturnTime',timing(end), ...
                    'Chart','legacy_named_cyclic','MinimumGap',0);
            end
            schedule.Times=timing(:);schedule.ReturnTime=timing(end);
