classdef HopperPlotPlugin < lmz.viz.PlotPlugin
    ...
    methods
        ...
        function value = kinematicsFrame(~, simulation, index)
            ...
            value = lmz.viz.KinematicsFrame(simulation.Time(index), ...
                index, frames, 'Vectors', vectors);
        end
        ...
        function handles = plot(~, axesHandle, descriptorId, simulation, ~)
            ...
            switch descriptorId
                case 'trajectory'
                    handles = plot(axesHandle, simulation.state('x'), ...
                        simulation.state('y'), 'LineWidth', 1.5);
                    xlabel(axesHandle, 'x'); ylabel(axesHandle, 'y');
                case 'states'
                    handles = plot(axesHandle, simulation.Time, ...
                        simulation.States, 'LineWidth', 1.2);
                    xlabel(axesHandle, 'time'); ylabel(axesHandle, 'state');
                otherwise
                    error('analytic_hopper:Plot', ...
